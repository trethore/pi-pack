...
export function createCompactPathFormatter(searchPaths: readonly string[]): (value: string) => string {
  ...
  return (value: string) => {
    ...
    if (root === undefined) return displayPath;
...
function formatRootedCompactPath(root: CompactRoot, relativePath: string, includeRootLabel: boolean): string {
  if (relativePath === '') return formatExactRootPath(root, includeRootLabel);
  if (!includeRootLabel || !root.label || root.label === '.') return relativePath;

  return `${root.label}/${relativePath}`;
}

function formatExactRootPath(root: CompactRoot, includeRootLabel: boolean): string {
  if (includeRootLabel && root.label !== '.') return root.label;
  return root.label || '.';
}
