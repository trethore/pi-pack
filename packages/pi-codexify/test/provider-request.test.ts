...
import { registerProviderRequest } from '#pi-codexify/provider-request.js';
...
describe('provider request mutations', () => {
  it('registers one hook for controls and web search', () => {
    ...
    registerProviderRequest(pi, () => config);
  ...
  it('does not mutate requests when the extension is disabled', () => {
    ...
    registerProviderRequest(pi, () => ({ ...defaultConfig, enabled: false }));
